			...
			<div class="filter-list">
				<form action="" method="get" class="form-horizontal" role="form" id="form1">
					...
					<div class="form-group" style="max-width: 1180px;">
						...
						<div class="col-sm-10">
							<div class="btn-group">
								...
								<a href="<?php  echo $this->createWebUrl('order',array('op'=>'tjorder','ac'=>'tjorder','ifpay'=>'1','keywordtype'=>$keywordtype,'keyword'=>$keyword,'timetype'=>$timetype,'start'=>$start,'end'=>$end,'hid'=>$_SESSION['hid']))?>" class="btn <?php  if($ifpay == '1') { ?> btn-primary <?php  } else { ?> btn-default <?php  } ?>">待体检</a>
								<a href="<?php  echo $this->createWebUrl('order',array('op'=>'tjorder','ac'=>'tjorder','ifpay'=>'2','keywordtype'=>$keywordtype,'keyword'=>$keyword,'timetype'=>$timetype,'start'=>$start,'end'=>$end,'hid'=>$_SESSION['hid']))?>" class="btn <?php  if($ifpay == '2') { ?> btn-primary <?php  } else { ?> btn-default <?php  } ?>">体检中</a>
					...
					<div class="form-group form-inline">
						<label class="col-sm-2 control-label">机构筛选</label>
						<div class="col-sm-9">
							<select name="h_id" class="form-control">
								<option value="">机构筛选</option>
								<?php  if(is_array($hos_arr)) { foreach($hos_arr as $hos) { ?>
								<option value="<?php  echo $hos['hid'];?>" <?php  if($hos['hid']==$h_id ) { ?> selected="" <?php  } ?>><?php  echo $hos['agentname'];?> </option>
								<?php  } } ?>
								</select>
						...
						<div class="tables">
							...
							<div class='table-row' style="margin:0  20px">
								...
								<a class="list-inner" data-toggle='popover' data-html='true' data-placement='right' data-trigger="hover" data-content="<table style='width:100%;'>
	                                    <tr>
	                                        <td  style='border:none;text-align:right;padding: 5px!important;'>机构实收：</td>
	                                        <td  style='border:none;text-align:right;padding: 5px!important;'>￥<?php  echo $item['moneys'];?></td>
	                                    </tr>
	           <tr>
	                                      ...
	                                      <tr>
	                                        ...
	                                        <td  style='border:none;text-align:right;color:green;padding: 5px!important;'>￥<?php  echo $item['moneys'];?></td>
	                                    </tr>
	
	                                </table>
	                    ">
									<div style='text-align:center'>
										￥<?php  echo $item['moneys'];?> </div>
								</a>
								...
								<div class="" style="overflow:visible;margin-top: 40px; text-align: center;">
									...
									<a class="btn btn-primary btn-sm" href="<?php  echo $this->createWebUrl('order',array('op'=>'tjorderxq','ac'=>'tjorderxq','id'=>$item['id'],'bm_id'=>$item['bm_id']))?>" title="">查看详情</a>
									<?php  if($item['ifpay'] =='0') { ?>
									<a class="btn btn-info btn-sm" href="<?php  echo $this->createWebUrl('order',array('op'=>'enter','ac'=>'enter','id'=>$item['id'],'hid'=>$_SESSION['hid']))?>">确定付款</a>
									<?php  } ?>
									<!-- <?php  echo $this->createWebUrl('ask',array('op'=>'askchat','ac'=>'askchat','id'=>$item['id']))?> -->
									<a class="btn btn-info btn-sm btnGoInfo" data-id="<?php  echo $item['id'];?>" data-bm_id="<?php  echo $item['bm_id'];?>" href="javascript:;">报告查看</a>
									<a class="btn btn-danger btn-sm" data-toggle="ajaxRemove" href="<?php  echo $this->createWebUrl('order',array('op'=>'tjorderdel','ac'=>'tjorderdel','id'=>$item['id'],'hid'=>$_SESSION['hid']))?>" data-confirm="确定要删除该记录吗？">快速删除</a>
    ...
    function showOrHide(a,b){
            ...
            $('#tab3 ul li').on('click',function(){
              ...
              $('.btnGoInfo').click(function(e){
                ...
                var url ="/index.php?c=site&a=entry&do=copysite&m=hyb_yl&act=profile.userbaogao&id="+id+'&bm_id='+bm_id
...
<script type="text/javascript">
    ...
    $('#de1').delegate('.pass_delete','click',function(e){
        ...
        if(!state)  return;
            $.post("<?php  echo $this->createWebUrl('order',array('op'=>'tjorderdels','ac'=>'tjorderdel'))?>", { ids : ids }, function(data){
                if(data.errno=='1'){ 
                    util.tips("操作成功！");
                    setTimeout(function(){ 
