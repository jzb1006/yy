								...
								<div class="list-inner paystyle">
									...
									<span>
										<i class="icow icow-yue text-warning" style="font-size: 13px;"></i>
											...
											<br />支付：<?php  if($item['ifpay'] =='1' || $item['ifpay'] =='2' || $item['ifpay'] =='3' || $item['ifpay'] =='4' || $item['ifpay'] =='5' || $item['ifpay'] =='6') { ?> <?php  echo $item['paytime'];?> <?php  } else if($item['ifgb'] =='1') { ?> [<?php  if($item['ifpay']=='1') { ?>已支付<?php  } else { ?>未支付<?php  } ?>]订单关闭 <?php  } else { ?>未支付 <?php  } ?>
								</div>
...
<script type="text/javascript">
	...
	$('#de1').delegate('.pass','click',function(e){
		...
		var $this = $(this);
		var ids = order_ids;
		var url="../web/index.php?c=site&a=entry&m=hyb_yl&op=del_telasks&ac=telask&do=ask&ids="+ids;
				console.log(ids);
		...
		if(!state) return;
			...
			$.post("../web/index.php?c=site&a=entry&m=hyb_yl&op=del_telasks&ac=telask&do=ask&", { ids : ids ,type:type}, function(data){
